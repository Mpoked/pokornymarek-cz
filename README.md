# pokornymarek.cz

Portfolio web pro Marka Pokorného — webdesigner z Uherského Hradiště.

Postaveno na **Next.js 15 · Tailwind CSS · TypeScript**.

---

## Co dodělat

### Obsah
- [ ] Doplnit reálné telefonní číslo (aktuálně `+420 000 000 000`)
- [ ] Doplnit skutečné reference / projekty (aktuálně jsou placeholdery s `localhost:3001`)
- [ ] Přidat fotku nebo logo do navbaru (místo textu „MP")
- [ ] Favicon a `og:image` pro sdílení na sociálních sítích

### Funkce
- [ ] Mobilní hamburger menu (nav je schovaný pod `md:flex`, na mobilu chybí)
- [ ] Animace při prvním načtení (hero sekce před sticky kartami)
- [ ] Přidat Google Analytics nebo Plausible

### Technické
- [ ] Nasadit na [Vercel](https://vercel.com) a připojit vlastní doménu `pokornymarek.cz`
- [ ] Nastavit environment proměnné pro Formspree ID (aktuálně natvrdo v kódu)
- [ ] Přidat `sitemap.xml` a `robots.txt` pro SEO
- [ ] Zkontrolovat Lighthouse skóre a vyladit Core Web Vitals

### Design
- [ ] Otestovat na různých prohlížečích (Safari, Firefox)
- [ ] Dark/light mode (volitelné)

---

## Spuštění lokálně

```bash
npm install
npm run dev
```

Otevři [http://localhost:3000](http://localhost:3000).
